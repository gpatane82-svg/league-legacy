      ...
      <article class="lol-lead-story">
        ...
        <div class="lol-verdict-grid">
          <article class="lol-verdict-card champion">
            <span>LEAGUE CHAMPION</span><img src="assets/florida/trophy.svg" alt=""><strong>${esc(championTeam||"PENDING")}</strong>
          ...
          <article class="lol-verdict-card sacko">
            <span>SACKO</span><img src="assets/florida/toilet.svg" alt=""><strong>${esc(sackoTeam||"PENDING")}</strong>
...
function floridaLeagueHistory(){
  ...
  const seasonFile=year=>{
    ...
    return `<article class="history-year-file">
      ...
      <div class="history-year-outcomes"><div class="history-year-stamp champion-stamp"><span>CHAMPION</span><strong>${esc(champName)}</strong><small>${esc(champ?.scoreText||league.champions.find(c=>c.season===year)?.record||"")}</small></div><div class="history-year-stamp sacko-stamp"><span>SACKO</span><strong>${esc(sackoName)}</strong><small>${esc(sacko?.scoreText||"")}</small></div></div>
    </article>`;
  };
...
function awardsCenter(){
  ...
  const titleCard=x=>`<article class="museum-plaque championship-plaque"><div class="museum-year">${x.year}</div><div class="museum-icon"><img src="assets/florida/gator-trophy.svg" alt=""></div><div class="museum-copy"><span>League Champion</span><h3>${ownerLinkByName(x.winner)}</h3><p>${esc(x.score)}</p><small>Defeated ${ownerLinkByName(x.runnerUp)}</small></div></article>`;
  const sackoCard=x=>`<article class="museum-plaque sacko-plaque"><div class="museum-year">${x.year}</div><div class="museum-icon"><img src="assets/florida/toilet.svg" alt=""></div><div class="museum-copy"><span>Sacko-Rolston Champ</span><h3>${ownerLinkByName(x.winner)}</h3><p>${esc(x.score)}</p><small>Lost to ${ownerLinkByName(x.runnerUp)}</small></div></article>`;
  const awards=annual?.awards||[];
  if(!isAllSeasons()) return `<section class="trophy-museum-hero paper-panel"><div class="museum-hero-copy"><span class="document-code">League of Losers Historical Society · ${state.season} Exhibit</span><p class="museum-kicker">The Trophy Case</p><h2>${latest?`${esc(latest.winner)} Takes the Crown`:`${state.season} Honors Await Certification`}</h2><p>${latest?`${ownerLinkByName(latest.winner)} defeated ${ownerLinkByName(latest.runnerUp)} in the championship, ${esc(latest.score)}.`:'No championship result is recorded for this season.'}</p></div><div class="museum-hero-object"><img src="assets/florida/gator-trophy.svg" alt="League gator trophy"><strong>${state.season}</strong><span>Championship Exhibit</span></div></section><div class="season-honors-grid"><article class="season-honor champion-honor paper-panel"><span class="honor-label">Champion</span><div class="honor-art"><img src="assets/florida/gator-trophy.svg" alt=""></div><h3>${latest?ownerLinkByName(latest.winner):'Pending'}</h3><p>${latest?`${esc(latest.score)} · Runner-up ${ownerLinkByName(latest.runnerUp)}`:'No championship event recorded'}</p></article><article class="season-honor sacko-honor paper-panel"><span class="honor-label">Sacko-Rolston Champ</span><div class="honor-art"><img src="assets/florida/toilet.svg" alt=""></div><h3>${latestSacko?ownerLinkByName(latestSacko.winner):'Pending'}</h3><p>${latestSacko?`${esc(latestSacko.score)} · Lost to ${ownerLinkByName(latestSacko.runnerUp)}`:'No Sacko event recorded'}</p></article></div><section class="paper-panel performance-gallery"><div class="panel-heading"><h2>${state.season} Performance Awards</h2></div>${awards.length?`<div class="award-ribbon-grid">${awards.map((a,i)=>`<article class="award-ribbon-card"><div class="award-medallion">${String(i+1).padStart(2,'0')}</div><span>${esc(a.label)}</span><h3>${a.ownerId?awardOwnerLink(a.ownerId,a.recipient):esc(a.recipient)}</h3><strong>${fmt.format(a.value)}</strong><small>${esc(a.detail)}</small></article>`).join('')}</div>`:`<div class="empty">No derived performance awards are available for ${state.season}.</div>`}</section>`;
  return `<section class="trophy-museum-hero paper-panel"><div class="museum-hero-copy"><span class="document-code">Permanent Collection · ${currentLeague().seasons[0]}–${latestSeason()}</span><p class="museum-kicker">The Trophy Case</p><h2>Glory, Shame & Questionable Decisions</h2><p>Every certified title and every Sacko-Rolston result, consolidated by permanent owner identity.</p><div class="museum-hero-stats"><span><strong>${documentedGames}</strong> documented games</span><span><strong>${allTitles.length}</strong> championships</span><span><strong>${allSackos.length}</strong> Sackos</span><span><strong>${champions.length}</strong> champions</span></div></div></section><section class="paper-panel champion-wall"><div class="panel-heading"><h2>Wall of Champions</h2></div><div class="champion-wall-grid dynamic-title-wall">${champions.map((c,i)=>{const rank=1+champions.filter(x=>x.count>c.count).length;return `<article class="champion-nameplate title-count-${Math.min(c.count,4)} ${c.count===maxTitles?'dynasty':''}"><span class="champion-rank">${String(rank).padStart(2,'0')}</span><div><h3>${managerLink(c.id,c.name)}</h3><p>${c.years.sort((a,b)=>b-a).join(' · ')}</p><small>${c.count} championship${c.count===1?'':'s'}</small></div><strong>${c.count}</strong></article>`}).join('')||'<div class="empty">No champions recorded.</div>'}</div></section><div class="museum-collection-grid"><section class="paper-panel collection-wing championship-wing"><div class="panel-heading"><h2>Championship Gallery</h2></div><div class="museum-plaque-grid">${titles.map(titleCard).join('')||'<div class="empty">No championship games recorded.</div>'}</div></section><section class="paper-panel collection-wing sacko-wing"><div class="panel-heading"><h2>Sacko-Rolston Champ</h2></div><div class="museum-plaque-grid">${sackos.map(sackoCard).join('')||'<div class="empty">No Sacko games recorded.</div>'}</div></section></div>`;
}
...
function floridaManagerFile(){
  ...
  <section class="paper-panel owner-milestones"><div class="panel-heading"><h2>Career Timeline</h2><span>Newest report first</span></div><div class="owner-timeline">${milestones.map(x=>`<article class="timeline-entry ${x.type}"><b>${x.year}</b><div><strong>${x.headline}</strong><span>${x.detail}</span></div></article>`).join("")}</div></section></div>
...
function commissionerOffice(){
  ...
  return `<section class="commissioner-office-v2">
    <header class="commissioner-hero commissioner-dossier">
      ...
      <aside class="commissioner-file-status" aria-label="Commissioner file status"><span>Official Working File</span><strong>LOL-09</strong><small>Images and league seal reserved for the artwork batch.</small></aside>
    </header>
